...
void CalculatePDFWeight(string dataSetName, double BDT, double MVA_weight_nom);
void FillGeneralPlots(int d, string prefix, vector <int> decayChannels, bool isData, bool toppair, double weight_);
...
int main(int argc, char* argv[]){
  ...
  for(int isys = 0; isys < thesystlist.size() ; isys++){
    ...
    for (int d = 0; d < datasets.size(); d++)   //Loop through datasets
    {
      ...
      for (int ievt = 0; ievt < endEvent; ievt++)
      {
        ...
        /// Fill plots
        if(doPDFunc){
          if(dataSetName.find("WZTo3LNu")!=std::string::npos) CalculatePDFWeight(dataSetName, BDT, MVA_weight_nom);
        }
        ...
        if (makePlots)
        {
          ...
          tempstring = region + "_"+coupling;
          if(isys != 0) tempstring += "_"+ systematic;
          FillGeneralPlots(d, tempstring, decayChannels, isData, toppair, 1.);
        }
      } // events
  ...
  // PUT HERE THE CODE TO CALCULATE ENVELOPE
...
void InitCalculatePDFWeightHisto(string dataSetName){
  ...
  for ( int i=0; i<101; i++)
  {
    output_histo_name = dataSetName+"_BDT_"+intToStr(i);
...
void CalculatePDFWeight(string dataSetName, double BDT, double MVA_weight_nom){
  ...
  for ( size_t i=0; i<newpdfSet.size(); i++)
  {
    ...
    output_histo_name = dataSetName+"_BDT_"+intToStr(i);
    histo1D[output_histo_name]->Fill(0.5, 1);
