...
if not os.path.exists("SubmitScripts_DiMuon"):
    os.makedirs("SubmitScripts_DiMuon")
...
for d in datasets:
    if d.attrib['add'] == '1':
        ...
        commandString = "./Selection_76X_Ntupler "+str(d.attrib['name'])+" "+str(d.attrib['title'])+" "+str(d.attrib['add'])+" "+str(d.attrib['color'])+" "+str(d.attrib['ls'])+" "+str(d.attrib['lw'])+" "+str(d.attrib['normf'])+" "+str(d.attrib['EqLumi'])+" "+str(d.attrib['xsection'])+" "+str(d.attrib['PreselEff'])
...
#        print "nb of files to be made: %d"  % len(topTrees)
        ...
        for f in range(0,len(topTrees)):
            filename="SubmitScripts_DiMuon/submit_"+str(d.attrib['name'])+str(N_file)+".sh"
