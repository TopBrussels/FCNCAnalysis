...
void MakePlot(string channel = "Elec_Elec", bool plotData = true) {
 ...
 cout << " - Rootfiles: " << endl; 
  ...
  else if(chan.find("mumu") == 0) dirname = "../_MuMu/";
...
//  {
//     cout << ((TObjString *)(oProc->At(4)))->String() << endl; 
//  } 
...
//  cout << "4" << endl;
 ...
 for(unsigned int i = 0; i<Vmyprocess.size(); i++) {
   ...
   // if(Vmyprocess[i].Contains("ST")){ color.push_back(kViolet);}
    //if(Vmyprocess[i].Contains("WJets")){ color.push_back(kCyan);} 
