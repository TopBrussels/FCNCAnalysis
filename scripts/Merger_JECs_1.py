...
for n in datasetNames:
    ...
    commandString_ntuples = "hadd -f " + pathdir_ntuples + "/FCNC_1L3B__Run2_TopTree_Study_" + n + "JESMinus.root"
...
for n in datasetNames:
    filenames_ntuples = glob.glob(inputdir_Ntuples + "/*" + n + "*.root")
    commandString_ntuples = "hadd -f " + pathdir_ntuples + "/FCNC_1L3B__Run2_TopTree_Study_" + n + "JESPlus.root"
    for ff in filenames_ntuples:
       if 'JESPlus' in ff:
          commandString_ntuples = commandString_ntuples + " " + ff
    #print "Merging control plots for " + str(d.attrib['name'])
...
for n in datasetNames:
    filenames_ntuples = glob.glob(inputdir_Ntuples + "/*" + n + "*.root")
    commandString_ntuples = "hadd -f " + pathdir_ntuples + "/FCNC_1L3B__Run2_TopTree_Study_" + n + "JERMinus.root"
    for ff in filenames_ntuples:
       if 'JERMinus' in ff:
          commandString_ntuples = commandString_ntuples + " " + ff
    #print "Merging control plots for " + str(d.attrib['name'])
...
for n in datasetNames:
    filenames_ntuples = glob.glob(inputdir_Ntuples + "/*" + n + "*.root")
    commandString_ntuples = "hadd -f " + pathdir_ntuples + "/FCNC_1L3B__Run2_TopTree_Study_" + n + "JERPlus.root"
    for ff in filenames_ntuples:
       if 'JERPlus' in ff:
          commandString_ntuples = commandString_ntuples + " " + ff
    #print "Merging control plots for " + str(d.attrib['name'])
