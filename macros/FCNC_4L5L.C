...
void FCNC_4L5L(){
  ...
  analyzer->addRealData("../ntuples/45_data_MuEG_RunA_tree.root", 19700); //19.7 fb¯1
  analyzer->addRealData("../ntuples/45_data_MuEG_RunB_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_MuEG_RunC_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_MuEG_RunD_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleMuParked_RunA_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleMuParked_RunB_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleMuParked_RunC_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleMuParked_RunD_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleElectron_RunA_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleElectron_RunB_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleElectron_RunC_tree.root", 19700);
  analyzer->addRealData("../ntuples/45_data_DoubleElectron_RunD_tree.root", 19700);
  ...
  analyzer->addMCSig("ZctL", "Zct", "../ntuples/45_TTJetsTocHbW_HToZZ_ZToBB_ZToLL_HctL_tree.root", 0.1575*scaler, -1, kRed+4, true);//cross section = 0.1575
  
  //analyzer->addMCSig("HctR", "Hct", "../ntuples/45_TTJetsTocHbW_HToZZ_ZToLL_HctL_tree.root", 5, -1, kRed, false);
  
  analyzer->addMCBkg("ggF", "SM_H", "../ntuples/45_GluGluHiggs4lep_tree.root", 19.27, -1, kBlue, ""); //cross section =  19.27pb
...
/*  analyzer->addMCBkg("ZZ_To4L", "ZZ", "../ntuples/45_ZZ_To4L_tree.root", 0.18, -1, kYellow, "");  // //cross section = 0.18
  ...
  */
  ...
  analyzer->addCutStep("InvMass_4lept_Zdecay > 0 && nJets>0","nJets,nLJets_CSVM,nBJets_CSVM,MET,OSSFpairs,FCNC_top,SM_lb,SM_W,SM_W_tr,SM_top",1);
