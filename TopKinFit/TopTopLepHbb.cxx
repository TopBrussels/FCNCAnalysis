// ttbar semileptonic with top quark FCNC decay to bbbar decay
#include "TopTopLepHbb.h"

ClassImp(KINFIT::TopTopLepHbb)

void fcnTopTopLepHbb(int &npar, double *gin, double &f, double *par, int iflag);

double funcTopTopLepHbb(double PxLepton,double PyLepton,double PzLepton,double ELepton,int LabelLepton,
...
KINFIT::TopTopLepHbb::TopTopLepHbb()
{
}
...
void KINFIT::TopTopLepHbb::TopTopLepHbbRun()
{
   ...
   if( TopTopLepHbb_ElectronIdx ) {delete[] TopTopLepHbb_ElectronIdx; TopTopLepHbb_ElectronIdx = 0;}
   if( TopTopLepHbb_MuonIdx ) {delete[] TopTopLepHbb_MuonIdx; TopTopLepHbb_MuonIdx = 0;}
   if( TopTopLepHbb_BJetLepIdx ) {delete[] TopTopLepHbb_BJetLepIdx; TopTopLepHbb_BJetLepIdx = 0;}
   if( TopTopLepHbb_NonBJetHadIdx ) {delete[] TopTopLepHbb_NonBJetHadIdx; TopTopLepHbb_NonBJetHadIdx = 0;}
   if( TopTopLepHbb_BJet1Idx ) {delete[] TopTopLepHbb_BJet1Idx; TopTopLepHbb_BJet1Idx = 0;}
   if( TopTopLepHbb_BJet2Idx ) {delete[] TopTopLepHbb_BJet2Idx; TopTopLepHbb_BJet2Idx = 0;}
   ...
   TopTopLepHbb_ElectronIdx = new int[NPERMEVENT];
   TopTopLepHbb_MuonIdx = new int[NPERMEVENT];
   TopTopLepHbb_BJetLepIdx = new int[NPERMEVENT];
   TopTopLepHbb_NonBJetHadIdx = new int[NPERMEVENT];
   TopTopLepHbb_BJet1Idx = new int[NPERMEVENT];
   TopTopLepHbb_BJet2Idx = new int[NPERMEVENT];
   ...
   for(int i=0;i<NPERMEVENT;i++)
     {	
	TopTopLepHbb_ElectronIdx[i] = -1;
	TopTopLepHbb_MuonIdx[i] = -1;
	TopTopLepHbb_BJetLepIdx[i] = -1;
	TopTopLepHbb_NonBJetHadIdx[i] = -1;
	TopTopLepHbb_BJet1Idx[i] = -1;
	TopTopLepHbb_BJet2Idx[i] = -1;
	chiPerm_[i] = 10E+10;
		  ...
		  for(int ib3=ib2+1;ib3<nBJet;ib3++)
		    {
			 ...
			 {
			    if( NoBTag_ && (ij == ib || ij == ib2 || ij == ib3) ) continue;
				 ...
				 if( res < disc_ )
				   {
				      ...
				      if( label_l == 0 )
					{
					   TopTopLepHbb_ElectronIdx[NPerm_] = idx_l;
					   TopTopLepHbb_MuonIdx[NPerm_] = -1;
					}							  
				      else if( label_l == 1 )
					{
					   TopTopLepHbb_MuonIdx[NPerm_] = idx_l;
					   TopTopLepHbb_ElectronIdx[NPerm_] = -1;
					}
				      
				      TopTopLepHbb_BJetLepIdx[NPerm_] = ib;
				      TopTopLepHbb_NonBJetHadIdx[NPerm_] = ij;
				      
				      TopTopLepHbb_BJet1Idx[NPerm_] = ib2;
				      TopTopLepHbb_BJet2Idx[NPerm_] = ib3;
...
void KINFIT::TopTopLepHbb::CalcNPerm()
{
   NPERMEVENT = 0;
...
void KINFIT::TopTopLepHbb::EmergencyAssign()
{
		  ...
		  for(int ib3=ib2+1;ib3<nBJet;ib3++)
		    {
		       ...
		       for(int ij=0;ij<nNonBJet;ij++)
			 {
			    ...
			    *PzNonBJet1 = NonBJetPz[ij];
			    ...
			    float mtopHad = pow(*EBJet2+*EBJet3+*ENonBJet1,2)-pow(*PxBJet2+*PxBJet3+*PxNonBJet1,2)-pow(*PyBJet2+*PyBJet3+*PyNonBJet1,2)-pow(*PzBJet2+*PzBJet3+*PzNonBJet1,2);
			    mTtop = sqrt(mTtop);
			    mtopHad = sqrt(mtopHad);
			    mHiggs = sqrt(mHiggs);
			    
			    float chi2 = pow(mTtop-*TopMass,2.)/pow(30,2.)+pow(mHiggs-125.,2.)/pow(15,2.)+pow(mtopHad-*TopMass,2.)/pow(30,2.);
			    ...
			    if( chi2 < chi2Min )
				 ...
				 if( label_l == 0 )
				   {
				      TopTopLepHbb_ElectronIdx[0] = idx_l;
				      TopTopLepHbb_MuonIdx[0] = -1;
				   }							  
				 else if( label_l == 1 )
				   {
				      TopTopLepHbb_MuonIdx[0] = idx_l;
				      TopTopLepHbb_ElectronIdx[0] = -1;
				   }
				 
				 TopTopLepHbb_BJetLepIdx[0] = ib;
				 TopTopLepHbb_NonBJetHadIdx[0] = ij;
				 
				 TopTopLepHbb_BJet1Idx[0] = ib2;
				 TopTopLepHbb_BJet2Idx[0] = ib3;
			      }		       
			 }		  
		    }	     
	       }	
			...
			double *par, double &proc)
{
   ...
   *KINFIT::kfit::PzNu1 = PzNu;
   ...
   float mHiggs = pow(par[19]+par[23],2)-pow(par[16]+par[20],2)-pow(par[17]+par[21],2)-pow(par[18]+par[22],2);
   ...
   chi2LeptonPz = 10E+10;
   chi2LeptonE = 10E+10;
   chi2NonBJetPxHad = 10E+10;
   chi2NonBJetPyHad = 10E+10;
   chi2NonBJetPzHad = 10E+10;
   chi2NonBJetEHad = 10E+10;
   chi2BJet1Px = 10E+10;
   chi2BJet1Py = 10E+10;
   chi2BJet1Pz = 10E+10;
   chi2BJet1E = 10E+10;
   ...
   if( mtop >= 0 && rac >= 0 && mtopHad >= 0 && mHiggs >= 0 && mW >= 0 )
     {
	mtop = sqrt(mtop);
	mW = sqrt(mW);
	...
	*KINFIT::kfit::TopMass2 = mtopHad;
	...
	float NonBJetPxHadProb = getProb(KINFIT::kfit::hPDFNonBJetPx.get(),par[12]-PxNonBJetHad);
	float NonBJetPyHadProb = getProb(KINFIT::kfit::hPDFNonBJetPy.get(),par[13]-PyNonBJetHad);
	float NonBJetPzHadProb = getProb(KINFIT::kfit::hPDFNonBJetPz.get(),par[14]-PzNonBJetHad);
	float NonBJetEHadProb = getProb(KINFIT::kfit::hPDFNonBJetE.get(),par[15]-ENonBJetHad);

	float BJet1PxProb = getProb(KINFIT::kfit::hPDFBJetPx.get(),par[16]-PxBJet1);
	float BJet1PyProb = getProb(KINFIT::kfit::hPDFBJetPy.get(),par[17]-PyBJet1);
	float BJet1PzProb = getProb(KINFIT::kfit::hPDFBJetPz.get(),par[18]-PzBJet1);
	float BJet1EProb = getProb(KINFIT::kfit::hPDFBJetE.get(),par[19]-EBJet1);

	float BJet2PxProb = getProb(KINFIT::kfit::hPDFBJetPx.get(),par[20]-PxBJet2);
	float BJet2PyProb = getProb(KINFIT::kfit::hPDFBJetPy.get(),par[21]-PyBJet2);
	float BJet2PzProb = getProb(KINFIT::kfit::hPDFBJetPz.get(),par[22]-PzBJet2);
	float BJet2EProb = getProb(KINFIT::kfit::hPDFBJetE.get(),par[23]-EBJet2);
	...
	chi2NonBJetPxHad = NonBJetPxHadProb;
	chi2NonBJetPyHad = NonBJetPyHadProb;
	chi2NonBJetPzHad = NonBJetPzHadProb;
	chi2NonBJetEHad = NonBJetEHadProb;
...
void KINFIT::TopTopLepHbb::calcVar(int iPerm)
{
//   if( chiPerm_[iPerm] > 10E+9 ) return;
   
   int idxElec = TopTopLepHbb_ElectronIdx[iPerm];
   int idxMuon = TopTopLepHbb_MuonIdx[iPerm];   
   int idxBJetLep = TopTopLepHbb_BJetLepIdx[iPerm];
   int idxNonBJetHad = TopTopLepHbb_NonBJetHadIdx[iPerm];
   int idxBJet1 = TopTopLepHbb_BJet1Idx[iPerm];
   int idxBJet2 = TopTopLepHbb_BJet2Idx[iPerm];
   ...
   float NonBJetHadPx = 0;
   float NonBJetHadPy = 0;
   float NonBJetHadPz = 0;
   float NonBJetHadE = 0;
   ...
   if( idxNonBJetHad >= 0 )
     {
	NonBJetHadPx = NonBJetPx[idxNonBJetHad];
	NonBJetHadPy = NonBJetPy[idxNonBJetHad];
	NonBJetHadPz = NonBJetPz[idxNonBJetHad];
	NonBJetHadE = NonBJetE[idxNonBJetHad];
     }   
   ...
   // Higgs
   float HiggsE = BJet1E+BJet2E;
   float HiggsPx = BJet1Px+BJet2Px;
   float HiggsPy = BJet1Py+BJet2Py;
   float HiggsPz = BJet1Pz+BJet2Pz;
   ...
   // topHad
   float topHadE = HiggsE+NonBJetHadE;
   float topHadPx = HiggsPx+NonBJetHadPx;
   float topHadPy = HiggsPy+NonBJetHadPy;
   float topHadPz = HiggsPz+NonBJetHadPz;
   ...
   WPt_[iPerm][1] = sqrt(HiggsPx*HiggsPx+HiggsPy*HiggsPy);
   WP_[iPerm][1] = sqrt(HiggsPx*HiggsPx+HiggsPy*HiggsPy+HiggsPz*HiggsPz);
   WEta_[iPerm][1] = getEta(WPt_[iPerm][1],HiggsPz);
   WRap_[iPerm][1] = getRap(HiggsE,HiggsPz);
   WPhi_[iPerm][1] = atan(HiggsPx/HiggsPy);
...
void fcnTopTopLepHbb(int &npar, double *gin, double &f, double *par, int iflag)
{
   ...
   double chi2LeptonPz;
   double chi2LeptonE;
   double chi2NonBJetPxHad;
   double chi2NonBJetPyHad;
   double chi2NonBJetPzHad;
   double chi2NonBJetEHad;
   double chi2BJet1Px;
   double chi2BJet1Py;
   double chi2BJet1Pz;
   double chi2BJet1E;
   ...
   double proc;
   double lh = funcTopTopLepHbb(*KINFIT::kfit::PxLepton1,
				...
				*KINFIT::kfit::EBJet3,
				chi2WLep, chi2TopLep,
				chi2Higgs, chi2TopHad,
				chi2EtMissX, chi2EtMissY,
				chi2BJetPxLep, chi2BJetPyLep, chi2BJetPzLep, chi2BJetELep,
				chi2NonBJetPxHad, chi2NonBJetPyHad, chi2NonBJetPzHad, chi2NonBJetEHad,
	...
	*KINFIT::kfit::CHISQ = lh;
	...
	(*KINFIT::kfit::ChiTerm).push_back(chi2NonBJetPxHad);
	(*KINFIT::kfit::ChiTerm).push_back(chi2NonBJetPyHad);
	(*KINFIT::kfit::ChiTerm).push_back(chi2NonBJetPzHad);
	(*KINFIT::kfit::ChiTerm).push_back(chi2NonBJetEHad);
...
void KINFIT::TopTopLepHbb::fit(double *chis,
			       double *par)
{
   ...
   *KINFIT::kfit::CHISQ = 10E+10;
   ...
   gMinuit->SetFCN(fcnTopTopLepHbb);
   ...
   gMinuit->mnparm(12,"NonBJetPxHad",par[12],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(13,"NonBJetPyHad",par[13],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(14,"NonBJetPzHad",par[14],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(15,"NonBJetEHad",par[15],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(16,"BJet1Px",par[16],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(17,"BJet1Py",par[17],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(18,"BJet1Pz",par[18],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(19,"BJet1E",par[19],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(20,"BJet2Px",par[20],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(21,"BJet2Py",par[21],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(22,"BJet2Pz",par[22],pow(10.,-1),-1000.,1000.,ierflg);
   gMinuit->mnparm(23,"BJet2E",par[23],pow(10.,-1),-1000.,1000.,ierflg);
...
void KINFIT::TopTopLepHbb::calcNuGrid(std::vector<double> &vp1,
				      std::vector<double> &vp2,
				      std::vector<double> &vp3,
				      std::vector<double> &vp4,
				      std::vector<double> &vp5,
				      ...
				      std::vector<double> &vchi2)
{
   ...
   for(int it=0;it<NToy_;it++)
	...
	do
	  {	     
	     par[12] = getProbGaus(hPDFNonBJetPx.get(),maxPDFNonBJetPx,meanPDFNonBJetPx,sigmaPDFNonBJetPx,rnd,NNonBJetPxRMS_,proc)+*KINFIT::kfit::PxNonBJet1;
	     par[13] = getProbGaus(hPDFNonBJetPy.get(),maxPDFNonBJetPy,meanPDFNonBJetPy,sigmaPDFNonBJetPy,rnd,NNonBJetPyRMS_,proc)+*KINFIT::kfit::PyNonBJet1;		  
	     par[14] = getProbGaus(hPDFNonBJetPz.get(),maxPDFNonBJetPz,meanPDFNonBJetPz,sigmaPDFNonBJetPz,rnd,NNonBJetPzRMS_,proc)+*KINFIT::kfit::PzNonBJet1;
	     par[15] = getProbGaus(hPDFNonBJetE.get(),maxPDFNonBJetE,meanPDFNonBJetE,sigmaPDFNonBJetE,rnd,NNonBJetERMS_,proc)+*KINFIT::kfit::ENonBJet1;
	  } while( (par[15]*par[15]-par[12]*par[12]-par[13]*par[13]-par[14]*par[14]) < 0. );	

	do
	  {	     
	     par[16] = getProbGaus(hPDFBJetPx.get(),maxPDFBJetPx,meanPDFBJetPx,sigmaPDFBJetPx,rnd,NBJetPxRMS_,proc)+*KINFIT::kfit::PxBJet2;
	     par[17] = getProbGaus(hPDFBJetPy.get(),maxPDFBJetPy,meanPDFBJetPy,sigmaPDFBJetPy,rnd,NBJetPyRMS_,proc)+*KINFIT::kfit::PyBJet2;		  
	     par[18] = getProbGaus(hPDFBJetPz.get(),maxPDFBJetPz,meanPDFBJetPz,sigmaPDFBJetPz,rnd,NBJetPzRMS_,proc)+*KINFIT::kfit::PzBJet2;
	     par[19] = getProbGaus(hPDFBJetE.get(),maxPDFBJetE,meanPDFBJetE,sigmaPDFBJetE,rnd,NBJetERMS_,proc)+*KINFIT::kfit::EBJet2;
	  } while( (par[19]*par[19]-par[16]*par[16]-par[17]*par[17]-par[18]*par[18]) < 0. );	

	do
	  {	     
	     par[20] = getProbGaus(hPDFBJetPx.get(),maxPDFBJetPx,meanPDFBJetPx,sigmaPDFBJetPx,rnd,NBJetPxRMS_,proc)+*KINFIT::kfit::PxBJet3;
	     par[21] = getProbGaus(hPDFBJetPy.get(),maxPDFBJetPy,meanPDFBJetPy,sigmaPDFBJetPy,rnd,NBJetPyRMS_,proc)+*KINFIT::kfit::PyBJet3;		  
	     par[22] = getProbGaus(hPDFBJetPz.get(),maxPDFBJetPz,meanPDFBJetPz,sigmaPDFBJetPz,rnd,NBJetPzRMS_,proc)+*KINFIT::kfit::PzBJet3;
	     par[23] = getProbGaus(hPDFBJetE.get(),maxPDFBJetE,meanPDFBJetE,sigmaPDFBJetE,rnd,NBJetERMS_,proc)+*KINFIT::kfit::EBJet3;
   ...
   vp13.push_back(*PxNonBJet1);
   vp14.push_back(*PyNonBJet1);
   vp15.push_back(*PzNonBJet1);
   vp16.push_back(*ENonBJet1);
   vp17.push_back(*PxBJet2);
   vp18.push_back(*PyBJet2);
   vp19.push_back(*PzBJet2);
   vp20.push_back(*EBJet2);
   vp21.push_back(*PxBJet3);
   vp22.push_back(*PyBJet3);
   vp23.push_back(*PzBJet3);
   vp24.push_back(*EBJet3);
