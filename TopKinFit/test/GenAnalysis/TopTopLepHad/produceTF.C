...
void produceTF()
{
   ...
   float TopLepWM, TopLepRecM, TopHadRecM, TopHadWRecM;
   ...
   ch.SetBranchAddress("dTopHadWNonBJet1Px",&dTopHadWNonBJet1Px);
   ch.SetBranchAddress("dTopHadWNonBJet1Py",&dTopHadWNonBJet1Py);
   ch.SetBranchAddress("dTopHadWNonBJet1Pz",&dTopHadWNonBJet1Pz);
   ch.SetBranchAddress("dTopHadWNonBJet1E",&dTopHadWNonBJet1E);
   ch.SetBranchAddress("dTopHadWNonBJet2Px",&dTopHadWNonBJet2Px);
   ch.SetBranchAddress("dTopHadWNonBJet2Py",&dTopHadWNonBJet2Py);
   ch.SetBranchAddress("dTopHadWNonBJet2Pz",&dTopHadWNonBJet2Pz);
   ch.SetBranchAddress("dTopHadWNonBJet2E",&dTopHadWNonBJet2E);
   ...
   ch.SetBranchAddress("dTopHadBJetPx",&dTopHadBJetPx);
   ch.SetBranchAddress("dTopHadBJetPy",&dTopHadBJetPy);
   ch.SetBranchAddress("dTopHadBJetPz",&dTopHadBJetPz);
   ch.SetBranchAddress("dTopHadBJetE",&dTopHadBJetE);
   ch.SetBranchAddress("TopLepWM",&TopLepWM);
   ch.SetBranchAddress("TopLepRecM",&TopLepRecM);
   ch.SetBranchAddress("TopHadRecM",&TopHadRecM);
   ch.SetBranchAddress("TopHadWRecM",&TopHadWRecM);
   ...
   h[nHist] = new TH1D("h_TopHadRecM","h_TopHadRecM",100,50.,300.);
   h[nHist]->Sumw2();
   hName[nHist] = "TopHadRecM";
   hLab[nHist] = "m(t) [GeV]";
   nHist++;

   h[nHist] = new TH1D("h_TopHadWRecM","h_TopHadWRecM",100,20.,140.);
   h[nHist]->Sumw2();
   hName[nHist] = "TopHadWRecM";
   hLab[nHist] = "m(W) [GeV]";
   nHist++;
   ...
   h[nHist] = new TH1D("h_dNonBJetPx","h_dNonBJetPx",100,-80.,80.);
   h[nHist]->Sumw2();
   hName[nHist] = "dNonBJetPx";
   hLab[nHist] = "NonBJetPx_{gen} - NonBJetPx_{reco} [GeV]";
   nHist++;

   h[nHist] = new TH1D("h_dNonBJetPy","h_dNonBJetPy",100,-80.,80.);
   h[nHist]->Sumw2();
   hName[nHist] = "dNonBJetPy";
   hLab[nHist] = "NonBJetPy_{gen} - NonBJetPy_{reco} [GeV]";
   nHist++;

   h[nHist] = new TH1D("h_dNonBJetPz","h_dNonBJetPz",100,-80.,80.);
   h[nHist]->Sumw2();
   hName[nHist] = "dNonBJetPz";
   hLab[nHist] = "NonBJetPz_{gen} - NonBJetPz_{reco} [GeV]";
   nHist++;

   h[nHist] = new TH1D("h_dNonBJetE","h_dNonBJetE",100,-300.,70.);
   h[nHist]->Sumw2();
	...
	for(int ih=0;ih<nHist;ih++)
	  {
	     ...
	     else if( hName[ih] == "TopHadWRecM" ) h[ih]->Fill(TopHadWRecM);
	     else if( hName[ih] == "dMetPx" ) h[ih]->Fill(dMetPx);
	     else if( hName[ih] == "dMetPy" ) h[ih]->Fill(dMetPy);
	     else if( hName[ih] == "dBJetPx" ) {h[ih]->Fill(dTopHadBJetPx);h[ih]->Fill(dTopLepBJetPx);}
	     else if( hName[ih] == "dBJetPy" ) {h[ih]->Fill(dTopHadBJetPy);h[ih]->Fill(dTopLepBJetPy);}
	     else if( hName[ih] == "dBJetPz" ) {h[ih]->Fill(dTopHadBJetPz);h[ih]->Fill(dTopLepBJetPz);}
	     else if( hName[ih] == "dBJetE" ) {h[ih]->Fill(dTopHadBJetE);h[ih]->Fill(dTopLepBJetE);}
	     ...
	     else if( hName[ih] == "dNonBJetPx" ) {h[ih]->Fill(dTopHadWNonBJet1Px);h[ih]->Fill(dTopHadWNonBJet2Px);}
	     else if( hName[ih] == "dNonBJetPy" ) {h[ih]->Fill(dTopHadWNonBJet1Py);h[ih]->Fill(dTopHadWNonBJet2Py);}
	     else if( hName[ih] == "dNonBJetPz" ) {h[ih]->Fill(dTopHadWNonBJet1Pz);h[ih]->Fill(dTopHadWNonBJet2Pz);}
	     else if( hName[ih] == "dNonBJetE" ) {h[ih]->Fill(dTopHadWNonBJet1E);h[ih]->Fill(dTopHadWNonBJet2E);}
	  }	
     }
...
//	h[i]->GetYaxis()->SetTitle("Normalized to unity");
	...
	if( hName[i] == "dMetPx" || hName[i] == "dMetPy" )
	  {
	     ...
	     std::string funcGausName = hName[i]+"_Gaus";
	     TF1 *funcGaus = new TF1(funcGausName.c_str(),"[6]*(gaus(0)+gaus(3))",h[i]->GetXaxis()->GetBinLowEdge(1),
				     h[i]->GetXaxis()->GetBinUpEdge(h[i]->GetXaxis()->GetNbins()));

	     funcGaus->FixParameter(6,1.0);
	     ...
	     fit->SetParameter(6,1./fit->GetMaximum());
	...
	if( hName[i] == "dBJetPx" || hName[i] == "dBJetPy" || hName[i] == "dBJetPz" || hName[i] == "dBJetE" )
	  {
	     ...
	     TF1 *funcGaus = new TF1(funcGausName.c_str(),"[6]*(gaus(0)+gaus(3)+gaus(6))",h[i]->GetXaxis()->GetBinLowEdge(1),
				     h[i]->GetXaxis()->GetBinUpEdge(h[i]->GetXaxis()->GetNbins()));
	     ...
	     if( hName[i] == "dBJetPx" || hName[i] == "dBJetPy" ) funcGaus->SetParameter(0,0.4);
	...
	if( hName[i] == "dNonBJetPx" || hName[i] == "dNonBJetPy" || hName[i] == "dNonBJetPz" || hName[i] == "dNonBJetE" )
	  {
	     double mean = 0.;
	     double sigma = 10.;
	     ...
	     funcGaus->SetParameter(1,mean);
	     funcGaus->SetParameter(2,sigma);
	     if( hName[i] == "dNonBJetPz" ) funcGaus->SetParameter(0,0.5);
	     if( hName[i] == "dNonBJetE" ) funcGaus->SetParameter(0,0.4);
	     if( hName[i] == "dNonBJetE" ) funcGaus->SetParameter(2,sigma*3);

	     funcGaus->SetParameter(3,0.3);
	     funcGaus->SetParameter(4,mean);
	     funcGaus->SetParameter(5,sigma*2.);
	     if( hName[i] == "dNonBJetPz" ) funcGaus->SetParameter(3,0.2);
	     if( hName[i] == "dNonBJetE" ) funcGaus->SetParameter(3,0.1);
	     if( hName[i] == "dNonBJetE" ) funcGaus->SetParameter(5,sigma*6);

	     funcGaus->SetParameter(6,1.);
	     funcGaus->SetParameter(7,mean);
	     funcGaus->SetParameter(8,sigma/2.);
	     if( hName[i] == "dNonBJetPz" ) funcGaus->SetParameter(8,sigma/3.);
	     if( hName[i] == "dNonBJetE" ) funcGaus->SetParameter(8,sigma);

	     funcGaus->SetParameter(9,0.5);
	     funcGaus->SetParameter(10,mean);
	     funcGaus->SetParameter(11,sigma);
	     if( hName[i] == "dNonBJetPz" ) funcGaus->SetParameter(9,0.);
	     
	     h[i]->Fit(funcGausName.c_str(),"QR");
	     TF1 *fit = h[i]->GetFunction(funcGausName.c_str());
	     fit->SetLineColor(1);
	     if( hName[i] == "dNonBJetE" )
	       {
		  c1->Update();
		  TPaveStats *st = (TPaveStats*)h[i]->FindObject("stats");
		  st->SetX1NDC(0.20);
	...
	if( hName[i] == "TopHadWRecM" || hName[i] == "TopHadRecM" || hName[i] == "TopLepRecM" )
	  {
	     double mean = 125.;
	     double sigma = 10.;
